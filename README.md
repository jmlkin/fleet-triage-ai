...
![Fleet Triage AI — fleet-health report](docs/screenshots/report.svg)

> Generated by `fleet-triage scan` on the committed synthetic fleet. Plain-text version: [`samples/report.txt`](samples/report.txt) · structured JSON: [`samples/fleet_health.json`](samples/fleet_health.json)
